"""ClearTrip (OTA) scraper for the airfare price index.

Deterministic mock-HTML parser mirroring the MMT vertical slice pattern.
Parses controlled HTML strings (never touches the real web) and extracts fare
data compatible with the ``RawFareSource`` contract.
...
This is NOT a live ClearTrip scraper: no network access, no CAPTCHA
...
#:
#: Uses Air India (AI) at a distinct price and distinct scrape time so the
#: integration test demonstrates correct coexistence with the MMT mock
#: (IndiGo 6E at 2026-08-27). The deduplicator key includes airline/price/
#: scraped_at but NOT source_name, so identical hardcoded values across
#: sources would be (correctly) collapsed as the same business observation.
_MOCK_FLIGHT_CARD = """
<div class="flight-card" data-route="DEL-BOM">
    <span class="price">₹5,499</span>
    <span class="cabin">Economy</span>
    <span class="airline">Air India (AI)</span>
    <span class="departure">08:30</span>
    <span class="date">2026-09-15</span>
    <a href="/flights/DEL-BOM/2026-09-15/AI-DEL-BOM">Book Now</a>
</div>
"""
...
class ClearTripScraper:
    """ClearTrip OTA scraper (prototype, mock-HTML parser).
    ...
    def _parse_card(self, card: str, index: int, route: Route | None) -> dict | None:
        ...
        return {
            ...
            "departure_at": "2026-09-15T08:30:00+00:00",
            ...
            "source": {
                ...
                "source_url": f"https://www.cleartrip.com/flights/{origin}-{destination}/2026-09-15",
                "raw_offer_id": f"CT-{origin}-{destination}-{index}",
            },
        }
