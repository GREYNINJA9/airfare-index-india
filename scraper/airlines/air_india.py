"""Air India (AI) Airline Direct Portal Scraper."""
...
<div class="flight-card" data-route="DEL-BOM">
    ...
    <a href="/booking/DEL-BOM/AI-101">Select</a>
</div>
"""
...
class AirIndiaScraper:
    """Air India direct airline portal scraper."""

    _templates: dict[str, str] = {"del_bom_economy": _MOCK_CARD}

    def __init__(self) -> None:
        self._name = "Air India"
    ...
    def extract(self, html: str, route: Route | None = None) -> List[dict]:
        ...
        try:
            ...
            if not fares:
                raise ExtractionError("No valid Air India fares extracted from HTML.")
        except Exception:
            return []
        return fares
    ...
    def _parse_card(self, card: str, index: int, route: Route | None) -> dict | None:
        ...
        return {
            ...
            "price_inr": price,
            "cabin_class": "ECONOMY",
            "departure_at": "2026-09-15T08:00:00+00:00",
            ...
            "source": {
                ...
                "raw_price": price,
                "raw_currency": "INR",
                "raw_cabin_label": "Economy Comfort",
                "source_url": f"https://www.airindia.com/booking/{origin}-{destination}",
                "raw_offer_id": f"AI-{origin}-{destination}-{index}",
            },
        }
