"""SpiceJet (SG) Airline Direct Portal Scraper."""
...
<div class="flight-card" data-route="DEL-BOM">
    ...
    <span class="departure">14:20</span>
    <span class="date">2026-09-15</span>
    <a href="/booking/DEL-BOM/SG-8169">Select</a>
</div>
"""
...
class SpiceJetScraper:
    ...
    def extract(self, html: str, route: Route | None = None) -> List[dict]:
        ...
        try:
            ...
            if not fares:
                raise ExtractionError("No valid SpiceJet fares extracted from HTML.")
        except Exception:
            return []
        return fares
    ...
    def _parse_card(self, card: str, index: int, route: Route | None) -> dict | None:
        ...
        return {
            ...
            "price_inr": price,
            "cabin_class": "ECONOMY",
            "departure_at": "2026-09-15T14:20:00+00:00",
            ...
            "source": {
                ...
                "raw_price": price,
                "raw_currency": "INR",
                "raw_cabin_label": "SpiceSaver",
                "source_url": f"https://www.spicejet.com/booking/{origin}-{destination}",
                "raw_offer_id": f"SG-{origin}-{destination}-{index}",
            },
        }
