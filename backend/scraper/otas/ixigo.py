...
<div class="flight-card" data-route="DEL-BOM">
    ...
    <span class="departure">12:30</span>
    <span class="date">2026-09-15</span>
    <a href="/flight/DEL-BOM/IXI-100">Book</a>
</div>
"""
...
class IxigoScraper:
    ...
    def extract(self, html: str, route: Route | None = None) -> List[dict]:
        ...
        try:
            ...
            if not fares:
                raise ExtractionError("No valid Ixigo fares extracted from HTML.")
        except Exception:
            return []
        return fares
    ...
    def _parse_card(self, card: str, index: int, route: Route | None) -> dict | None:
        ...
        return {
            ...
            "price_inr": price,
            "cabin_class": "ECONOMY",
            "departure_at": "2026-09-15T12:30:00+00:00",
            ...
            "source": {
                ...
                "raw_price": price,
                "raw_currency": "INR",
                "raw_cabin_label": "Standard",
                "source_url": f"https://www.ixigo.com/flights/{origin}-{destination}",
                "raw_offer_id": f"IXI-{origin}-{destination}-{index}",
            },
        }
