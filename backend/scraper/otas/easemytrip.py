...
<div class="flight-card" data-route="DEL-BOM">
    ...
    <span class="departure">10:15</span>
    <span class="date">2026-09-15</span>
    <a href="/flight/DEL-BOM/EMT-100">Book</a>
</div>
"""
...
class EaseMyTripScraper:
    ...
    def extract(self, html: str, route: Route | None = None) -> List[dict]:
        ...
        try:
            ...
            if not fares:
                raise ExtractionError("No valid EaseMyTrip fares extracted from HTML.")
        except Exception:
            return []
        return fares
    ...
    def _parse_card(self, card: str, index: int, route: Route | None) -> dict | None:
        ...
        return {
            ...
            "price_inr": price,
            "cabin_class": "ECONOMY",
            "departure_at": "2026-09-15T10:15:00+00:00",
            ...
            "source": {
                ...
                "source_url": f"https://www.easemytrip.com/flights/{origin}-{destination}",
                "raw_offer_id": f"EMT-{origin}-{destination}-{index}",
            },
        }
